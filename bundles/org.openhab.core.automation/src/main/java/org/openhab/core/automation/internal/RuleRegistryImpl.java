...
public class RuleRegistryImpl extends AbstractRegistry<Rule, String, RuleProvider>
    ...
    private static final Set<TemplateState> PROCESSABLE_TEMPLATE_STATES = Set.of(TemplateState.PENDING, TemplateState.TEMPLATE_MISSING);
    ...
    public void regenerateFromTemplate(String ruleUID) {
        ...
        if (rule == null) {
            throw new IllegalArgumentException("Can't regenerate rule from template because no rule with UID \"" + ruleUID + "\" exists");
        }
        if (rule.getTemplateUID() == null || rule.getTemplateState() == TemplateState.NO_TEMPLATE) {
            throw new IllegalArgumentException("Can't regenerate rule from template because the rule isn't linked to a template");
        }
        try {
            Rule resolvedRule = resolveRuleByTemplate(RuleBuilder.create(rule).withActions((List<Action>) null)
                    .withConditions((List<Condition>) null).withTriggers((List<Trigger>) null)
                    .withTemplateState(TemplateState.PENDING).build());
            ...
            logger.error("Regenerating rule '{}' from template failed: " + e.getMessage(), rule.getUID(), e);
            throw e;
        }
    }
    ...
    protected void processRuleStubs(RuleTemplate template) {
        ...
        for (Rule unresolvedRule : rules) {
            try {
                ...
                logger.error("Resolving the rule '{}' by template '{}' failed", unresolvedRule.getUID(), templateUID, e);
            }
        }
    }
}
